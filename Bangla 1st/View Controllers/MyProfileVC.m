...
@interface MyProfileVC ()<UITableViewDelegate,UITableViewDataSource>
{
    ...
    BOOL isAccountListWebServiceCalled;
...
-(void)getPackageListing
{
    ...
    [[PackageListingWebService service]callPackageListingWebServiceWithDictParams:packageListDict success:^(id  _Nullable response, NSString * _Nullable strMsg) {
        ...
        if (response != nil)
        {
            ...
            {
                objModelCouponData = [[ModelCouponData alloc]initWithDictionary:response[@"coupon_data"][0]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else       //if (indexPath.section == 2)
    {
        ...
        if (isAccountListWebServiceCalled)
        {
            cell.btnUpgradeOutlet.hidden = NO;
            
            if ([self.appDel.objModelUserInfo.strUserIsSubscribed isEqualToString:@"yes"])
            {
                isAccountListWebServiceCalled = NO;
                ...
                if (objModelCouponData !=nil)
                {
                    ...
                    cell.lblPackageName.text = @"Coupon Code Subscription";
                    cell.lblPackagePrice.text = [NSString stringWithFormat:@"Rs. %@ /day",objModelCouponData.strCouponPackagePrice];
                    cell.lblPackageExpiryDt.text = [NSString stringWithFormat:@"Expires on %@",objModelCouponData.strCouponSubscriptionEndDate];

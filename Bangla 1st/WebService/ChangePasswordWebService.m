...
+(id)service
{
    static ChangePasswordWebService *service;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        service=[[ChangePasswordWebService alloc]initWithService:WEB_SERVICES_CHANGE_PASSWORD];
    });
    return service;
}
...
-(void)callChangePasswordWebServiceWithDictParams:(NSDictionary *)dictParams success:(success)successHandler failure:(failure)failureHandler
{
    if (appDel.isRechable)
    {
        ...
        {
            NSLog(@"Exception in Change Password Webservice:%@",anException);
            
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"Could not execute Change Password Webservice because an Exception occured."};

...
@implementation AccountDetailsWebService

+(id)service
{
    static AccountDetailsWebService *service;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        service=[[AccountDetailsWebService alloc]initWithService:WEB_SERVICES_ACCOUNT_DETAILS];
    });
    return service;
}
...
-(void)callgetAccountDetailsWebServiceWithDictParams:(NSDictionary *)dictParams success:(success)successHandler failure:(failure)failureHandler
{
    if (appDel.isRechable)
    {
        ...
        {
            NSLog(@"Exception in Account Details Webservice:%@",anException);
            
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"Could not execute Account Details Webservice because an Exception occured."};
